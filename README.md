# Bulgarian National Bank Exchange Rate API client

Official **Bulgarian National Bank** (Bulgaria) daily exchange rates in Node.js / TypeScript — ~29 currencies against the EUR, with history back to 2016. Zero dependencies, works in Node 18+, Bun, Deno, and edge runtimes (uses global `fetch`).
...
Powered by [AllRatesToday](https://allratestoday.com/central-bank-rates-api/bnb/). Get a free API key at [allratestoday.com/register](https://allratestoday.com/register) — 300 requests/month, no credit card.
...
```js
import { getRate, getLatestRates } from 'bnb-exchange-rate';

// One pair at the official Bulgarian National Bank rate
const pair = await getRate('EUR', 'USD', { apiKey: 'art_live_...' });
console.log(pair.rate, pair.rate_date); // e.g. EUR -> USD on the bank's own date
...
```js
import { getRatesForDate, getHistory } from 'bnb-exchange-rate';
...
const series = await getHistory(
  { source: 'EUR', target: 'USD', from: '2026-01-01' },
  { apiKey: 'art_live_...' }
);
```
...
If Bulgarian National Bank does not print a pair directly, the API resolves it from the bank's table (inverse, or a cross rate via EUR) and flags it `derived: true` with the `method` — so official and computed values are never confused.
